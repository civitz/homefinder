...
from database import DatabaseManager, TelegramConfiguration, NotificationSubscription, NotificationHistory
...
class TelegramService:
    ...
    def send_notification(self, config_id: int, message: str, listing_url: str, 
                         bot_token: Optional[str] = None, chat_id: Optional[str] = None) -> bool:
        """Send notification to Telegram chat."""
        if self.dry_run:
            self.logger.info(f"[DRY RUN] Would send Telegram notification: {message[:50]}...")
            return True
        else:
            # If bot_token and chat_id are provided directly, use them for testing
            if bot_token and chat_id:
                return self._send_direct_notification(bot_token, chat_id, message, listing_url)
            else:
                return self._send_real_notification(config_id, message, listing_url)
             
    def _send_direct_notification(self, bot_token: str, chat_id: str, message: str, listing_url: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('ok', False):
                    self.logger.info(f"Successfully sent direct Telegram notification to chat {chat_id}")
                    return True
                else:
                    error_code = result.get('error_code')
                    error_message = result.get('description', 'Unknown error')
                    self.logger.error(f"Telegram API error {error_code}: {error_message}")
                    return False
            else:
                self.logger.error(f"Telegram API HTTP error {response.status_code}: {response.text}")
                return False
    ...
    def _send_real_notification(self, config_id: int, message: str, listing_url: str) -> bool:
        ...
        try:
            ...
            if not config.chat_id:
                self.logger.warning(f"Telegram configuration {config_id} has no chat_id")
                return False
            ...
            if response.status_code == 200:
                result = response.json()
                if result.get('ok', False):
                    self.logger.info(f"Successfully sent Telegram notification to chat {chat_id}")
                    return True
                else:
                    error_code = result.get('error_code')
                    error_message = result.get('description', 'Unknown error')
    ...
    def _get_telegram_config_by_id(self, config_id: int) -> Optional[TelegramConfiguration]:
...
class NotificationEngine:
    ...
    def check_new_listings(self, since_timestamp: Optional[str] = None) -> List[Listing]:
        ...
        try:
            ...
            return [l for l in listings if l.id is not None and not self._was_notified_about(l.id)]
        except Exception as e:
            ...
            for listing in listings:
                for subscription in subscriptions:
                    if self._listing_matches_subscription(listing, subscription):
                        ...
                        self.logger.info(f"Listing {listing.url} matches subscription {subscription.subscription_name}")
    ...
    def _listing_matches_subscription(self, listing: Listing, subscription: NotificationSubscription) -> bool:
        ...
        try:
            ...
            for filter_key, filter_value in subscription.search_filters.items():
                if filter_key == 'city' and listing_dict.get('city') != filter_value:
                    ...
                    return False
                elif filter_key == 'min_size' and listing_dict.get('square_meters', 0) < filter_value:
                    return False
    ...
    def send_notifications(self, matches: List[Dict]) -> int:
        """Send notifications for matched listings."""
        success_count = 0
        ...
        for match in matches:
            ...
            try:
                ...
                self.db_manager.log_notification(
                    ...
                    telegram_message_id="mock_id" if success else None,
                ...
                if success:
                    success_count += 1
                    self.logger.info(f"Successfully sent notification for listing {listing.url}")
                else:
                    self.logger.warning(f"Failed to send notification for listing {listing.url}")
                    
            except Exception as e:
                self.logger.error(f"Error sending notification for listing {listing.url}: {e}")
    ...
    def run_periodic_check(self, poison_pill: Any = None) -> None:
        ...
        while True:
            ...
            if poison_pill and hasattr(poison_pill, 'is_set') and poison_pill.is_set():
                self.logger.info("Received poison pill, shutting down notification service")
                break
            
            try:
                # Process new listings
                sent_count = self.process_new_listings()
                
                if sent_count > 0:
                    self.logger.info(f"Successfully sent {sent_count} notifications")
                
                # Sleep for 60 seconds before next check
                time.sleep(60)
                
            except KeyboardInterrupt:
                self.logger.info("Received keyboard interrupt, shutting down notification service")
                break
            except Exception as e:
    ...
    def _format_notification_message(self, listing: Listing, subscription_name: str = "") -> str:
        ...
        try:
            ...
            message += f"📝 *Description*: {listing.description[:100]}..." if listing.description else ""
