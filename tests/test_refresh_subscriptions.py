...
def test_get_notification_subscriptions_by_user():
    ...
    try:
        ...
        for user_id in users:
            ...
            print(f"✓ User {user_id}: {len(user_subscriptions)} subscriptions retrieved correctly")
        
        # Test retrieving subscriptions for non-existent user
        empty_subscriptions = db.get_notification_subscriptions_by_user('nonexistent_user')
