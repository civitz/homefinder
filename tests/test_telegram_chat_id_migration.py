...
def test_telegram_chat_id_migration():
    ...
    try:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', ('test_user', 'Test Subscription', '{"city": "Padova"}', config_id, True, datetime.now().isoformat(), datetime.now().isoformat()))
        ...
        new_subscription_id = db_manager.save_notification_subscription(new_subscription)
