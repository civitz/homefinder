...
class BackgroundScraper:
    ...
    def __init__(
        ...
        stop_signal: Optional[AtomicBool] = None,
        ...
        if scrapers is not None:
            ...
            self.scrapers = [
                TettorossoScraper(
                    request_delay_ms=request_delay_ms, stop_signal=stop_signal
                ),
                GalileoScraper(
                    request_delay_ms=request_delay_ms, stop_signal=stop_signal
                ),
    ...
    def should_run_scrape(self, force: bool = False) -> bool:
        ...
        if time_since_last_scrape.total_seconds() >= self.interval_seconds:
            self.logger.info(
                f"Last scrape was {time_since_last_scrape.total_seconds():.1f} seconds ago, will run scraping"
            )
            return True
        else:
            remaining_seconds = (
                self.interval_seconds - time_since_last_scrape.total_seconds()
            )
            self.logger.info(
                f"Last scrape was {time_since_last_scrape.total_seconds():.1f} seconds ago, skipping (next in {remaining_seconds:.1f}s)"
            )
            return False
    ...
    def _scrape_all_websites(self, force: bool = False) -> int:
        ...
        try:
            ...
            self.db_manager.log_scrape_run(
                "background", total_listings, duration_seconds
            )

            self.logger.info(
                f"Scraping run completed. Total listings: {total_listings} in {duration_seconds:.1f} seconds"
            )
        ...
        while self.running:
            try:
                ...
                for _ in range(int(sleep_time)):
                    ...
                    if self.stop_signal and self.stop_signal.load():
                        self.logger.info(
                            "Background scraper stopping due to poison pill"
                        )
                        self.running = False
                        break
                    time.sleep(1)
    ...
    def start(self):
        ...
        self.running = True
        self.logger.info(
            f"Starting background scraper with {self.interval_seconds} second interval"
        )
