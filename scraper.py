...
class BaseScraper:
    def _setup_file_logger(self):
        """Setup file logger with daily rotation."""
    ...
    def __init__(
        ...
        self.request_delay_ms = (
            request_delay_ms if request_delay_ms is not None else REQUEST_DELAY_MS
        )
    ...
    def _should_stop(self) -> bool:
        ...
        if self.stop_signal and self.stop_signal.load():
            self.logger.info(
                "Poison pill detected, stopping gracefully after current listing"
            )
            return True
        return False
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt == 0 and self.request_delay_ms > 0:
                    ...
                    self.logger.debug(
                        f"Adding {self.request_delay_ms}ms delay before next request"
                    )
...
class TettorossoScraper(BaseScraper):
    ...
    def scrape_live_listings(self) -> List[Listing]:
        ...
        try:
            ...
            if not html_content:
                self.logger.warning(
                    f"Failed to fetch Tettorosso listings page: {listings_page_url}"
                )
                return listings
            ...
            while has_more_pages:
                self.logger.info(
                    f"Fetching Tettorosso properties - Page {current_page}"
                )

                # Call the WordPress AJAX endpoint to get properties
                ajax_url = (
                    f"{self.base_url}/wp-admin/admin-ajax.php?action=sf_get_immobili"
                )
                ...
                try:
                    ...
                    response = self.session.post(
                        ajax_url, data=form_data, timeout=REQUEST_TIMEOUT
                    )
                    response.raise_for_status()
                    ...
                    if not ajax_data.get("success"):
                        self.logger.warning(
                            f"AJAX request failed for page {current_page}"
                        )
                    ...
                    for element in link_elements:
                        ...
                        if href and "/immobili/" in href and href.count("/") >= 3:
                            ...
                            if (
                                href.endswith("/immobili/")
                                or "/immobili/page/" in href
                                or href == "/immobili/"
                            ):
                                continue
                    ...
                    self.logger.info(
                        f"Found {len(property_links)} property links on page {current_page}"
                    )
                        ...
                        try:
                            ...
                            if property_html:
                                ...
                                if listing:
                                    ...
                                    self.logger.info(
                                        f"Successfully scraped Tettorosso: {listing.title}"
                                    )
                                else:
                                    self.logger.warning(
                                        f"Failed to parse Tettorosso property: {link}"
                                    )
                            else:
                                self.logger.warning(
                                    f"Failed to fetch Tettorosso property page: {link}"
                                )
                    ...
                    self.logger.error(
                        f"AJAX request failed for page {current_page}: {e}"
                    )
                    has_more_pages = False
                    break
...
class GalileoScraper(BaseScraper):
    ...
    def _extract_agency_listing_id(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        if property_id_input:
            return (
                property_id_input.get("value", "").strip()
                if property_id_input.get("value")
                else None
            )
        ...
        for listing_type in listing_types:
            ...
            while has_more_pages:
                ...
                for element in listing_elements:
                    ...
                    if (
                        href
                        and f"/{listing_type}/" in href
                        and href.count("/") > 2
                        and "/page/" not in href
                        and not href.endswith(f"/{listing_type}/")
                    ):
                        if not href.startswith("http"):
                            href = (
                                f"{self.base_url}{href}"
                ...
                self.logger.info(
                    f"Found {len(property_links)} property links on page {page}"
                )
                    ...
                    try:
                        ...
                        if property_html:
                            ...
                            if listing:
                                listings.append(listing)
                                self.logger.info(
                                    f"Successfully scraped: {listing.title}"
                                )
